locals {
  app_fqdn = { for k in var.env : k => "app.${trim(var.dns_zone[k].dns_name, ".")}" }
}

# Create an SA for the MIG instances
resource "google_service_account" "frontend-mig-sa" {
  for_each     = toset(var.env)
  project      = var.frontend_projects[each.key].name
...
resource "google_compute_instance_template" "frontend-instance-template" {
  ...
  network_interface {
    ...
    subnetwork         = var.subnet_id[each.key].frontend
...
resource "google_certificate_manager_certificate_map" "frontend-certmap" {
  ...
  project     = var.frontend_projects[each.key].name
  name        = "fe-certmap"
  description = "Certificate map for ${local.app_fqdn[each.key]}"
}
...
resource "google_certificate_manager_certificate_map_entry" "frontend-certmap-entry" {
  ...
  project      = var.frontend_projects[each.key].name
  name         = "certmap-entry"
  description  = "Cert Manager map entry for ${local.app_fqdn[each.key]}"
...
resource "google_certificate_manager_certificate" "frontend-certmap-certificate" {
  ...
  description = "Cert Manager certificate for ${local.app_fqdn[each.key]}"
  scope       = "DEFAULT"
  managed {
    domains            = [local.app_fqdn[each.key]]
...
resource "google_certificate_manager_dns_authorization" "frontend-dns-auth" {
  ...
  project     = var.frontend_projects[each.key].name
  name        = "fe-dns-auth"
  description = "Cert Manager authorization for ${local.app_fqdn[each.key]}"
...
resource "google_dns_record_set" "dns-auth-entry" {
  ...
  managed_zone = var.dns_zone[each.key].name
  ttl          = 300
}
...
resource "google_dns_record_set" "dns-app-entry" {
  for_each     = toset(var.env)
  project      = var.network_projects[each.key].name
  name         = "app.${var.dns_zone[each.key].dns_name}"
  managed_zone = var.dns_zone[each.key].name
  type         = "A"
  ttl          = 300
